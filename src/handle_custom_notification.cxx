...
ptr<buffer> custom_notification_msg::serialize() const {
    ...
    size_t len = sizeof(uint8_t) + sizeof(uint8_t) + sizeof(uint32_t) + ((ctx_) ? ctx_->size() : 0);
...
ptr<resp_msg> raft_server::handle_custom_notification_req(req_msg& req) {
    ptr<resp_msg> resp = cs_new<resp_msg>(
        state_->get_term(), msg_type::custom_notification_response, id_, req.get_src(), log_store_->next_slot());
...
ptr<resp_msg>
raft_server::handle_leadership_takeover(req_msg& req, ptr<custom_notification_msg> msg, ptr<resp_msg> resp) {
    if (is_leader()) {
...
ptr<resp_msg>
raft_server::handle_resignation_request(req_msg& req, ptr<custom_notification_msg> msg, ptr<resp_msg> resp) {
    if (!is_leader()) {
