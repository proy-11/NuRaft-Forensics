...
private:
        ...
        try {
            ...
            if (hdr->get_int() > 0 && log_ctx) {
                ...
                while (log_ctx->size() > log_ctx->pos()) {
                    ...
                    ptr<log_entry> entry(
                        cs_new<log_entry>(term, buf, val_type, prev, sig));
    ...
    void on_resp_ready(ptr<req_msg> req, ptr<resp_msg> resp) {
        ...
        try {
            ...
            size_t carried_data_size =
                resp_sig_size + resp_meta_size + resp_hint_size + resp_ctx_size;
...
public:
    ...
    virtual void send(ptr<req_msg>& req,
        ...
        for (auto& entry: req->log_entries()) {
            ...
            // FMARK:
            entry_buf->put(le->get_prev_ptr() == nullptr
                               ? (int32)0
                               : (int32)le->get_prev_ptr()->size());
            if (le->get_prev_ptr()) {
        ...
        ptr<buffer> req_buf =
            buffer::alloc(RPC_REQ_HEADER_SIZE + meta_size + cert_size + log_data_size);
