...
void raft_server::request_append_entries() {
    ...
    if (peers_.size() == 0 || get_quorum_for_commit() == 0) {
        p_tr("append entries requested for one node cluster or quorum size 1 (including "
             "leader)");
...
bool raft_server::request_append_entries(ptr<peer> p) {
    p_tr("append entries requested for peer id: %d, next log idx: %llu",
         p->get_id(),
         p->get_next_log_idx());
    ...
    if (params->auto_adjust_quorum_for_small_cluster_ && get_num_voting_members() == 2
        ...
        } else if (num_not_responding_peers == 0 && num_stale_peers == 0
    ...
    if (last_active_time_ms
        > params->heart_beat_interval_ * raft_server::raft_limits_.reconnect_limit_) {
    ...
    if (p->make_busy()) {
        ...
        p_db("NUM SEND ENTRIES = %llu", entries.size());
        // for (size_t k = 0; k < entries.size(); k++) {
        //     p_wn("SEND REQ ENTRY %lld (%d): %s (%s)",
        //          k,
        //          entries[k]->get_val_type(),
        //          tobase64(*create_hash(entries[k], k + msg->get_last_log_idx() +
        //          1)).c_str(), entries[k]->get_val_type() == log_val_type::app_log
...
ptr<req_msg> raft_server::create_append_entries_req(peer& p) {
    ...
    if (log_entries) {
        ...
        if (flag_use_leader_sig()) {
            // get the last app_log entry in log_entries
            ptr<log_entry> last_app_log = nullptr;
            for (auto it : *log_entries) {
                if (it->get_val_type() == log_val_type::app_log) {
                    last_app_log = it;
                    break;
                }
            }
            if (last_app_log != nullptr) {
                ...
                ptr<log_entry> leader_sig_le =
                    cs_new<log_entry>(0, this->get_signature(*last_app_log->serialize_sig()), log_val_type::leader_sig);
                v.push_back(leader_sig_le);
                p_in("leader signature sent to peer %d", p.get_id());
            } else {
                p_in("no app_log entry found in log_entries, leader signature not sent to peer %d", p.get_id());
            }
        }
...
ptr<resp_msg> raft_server::handle_append_entries(req_msg& req) {
    ...
    bool log_okay =
        req.get_last_log_idx() == 0 || (log_term && req.get_last_log_term() == log_term)
    ...
    if (req.log_entries().size() > 0
        && req.log_entries().at(0)->get_val_type() == log_val_type::hash_ptr) {
        hash_ptr = buffer::clone(*req.log_entries().at(0)->get_buf_ptr());
        p_in("hash pointer (%s) received from peer %d",
             tobase64(*hash_ptr).c_str(),
             req.get_src());
    ...
    if (log_size > 0) {
        ...
        if (flag_use_ptr() && hash_ptr != nullptr) {
            ...
            if ((conflict = match_log_entry(req.log_entries(), prev_idx + 1, hash_ptr))
                != true) {
                ...
                p_db("req log idx %zu -- %zu passed pointer checks",
                     log_idx,
                     log_idx + log_size - 1);
            }
        ...
        if (flag_use_leader_sig() && leader_sig != nullptr) {
            if (last_app_log == nullptr) {
                p_wn("no app_log entry found in log_entries, but leader signature provided (must be a bug!)");
            } else {
                if ((conflict = check_leader_sig(last_app_log, leader_sig, req.get_src())) == false) {
                    p_wn("deny illegal signature of requests appending %zu: item %zu",
                        log_idx + 1 + conflict);
...
bool raft_server::try_update_precommit_index(ulong desired, const size_t MAX_ATTEMPTS) {
    ...
    while (prev_precommit_index < desired && num_attempts < MAX_ATTEMPTS) {
...
void raft_server::handle_append_entries_resp(resp_msg& resp) {
    ...
    if (resp.get_accepted()) {
        ...
        if (flag_use_cc() && resp.get_signature() != nullptr) {
            ...
            push_new_cert_signature(resp.get_signature(),
                                    p->get_id(),
                                    entry->get_term(),
                                    resp.get_sig_index());
