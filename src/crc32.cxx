...
const uint32_t crc_lookup[8][256] = {
    ...
    // for (int i = 0; i <= 0xFF; i++)
    //{
    //  Crc32Lookup[1][i] = (Crc32Lookup[0][i] >> 8) ^ Crc32Lookup[0][Crc32Lookup[0][i] &
    //  0xFF];
    //  Crc32Lookup[2][i] = (Crc32Lookup[1][i] >> 8) ^ Crc32Lookup[0][Crc32Lookup[1][i] &
    //  0xFF];
    //  Crc32Lookup[3][i] = (Crc32Lookup[2][i] >> 8) ^ Crc32Lookup[0][Crc32Lookup[2][i] &
    //  0xFF];

    //  Crc32Lookup[4][i] = (Crc32Lookup[3][i] >> 8) ^ Crc32Lookup[0][Crc32Lookup[3][i] &
    //  0xFF];
    //  Crc32Lookup[5][i] = (Crc32Lookup[4][i] >> 8) ^ Crc32Lookup[0][Crc32Lookup[4][i] &
    //  0xFF];
    //  Crc32Lookup[6][i] = (Crc32Lookup[5][i] >> 8) ^ Crc32Lookup[0][Crc32Lookup[5][i] &
    //  0xFF];
    //  Crc32Lookup[7][i] = (Crc32Lookup[6][i] >> 8) ^ Crc32Lookup[0][Crc32Lookup[6][i] &
    //  0xFF];
    //}
...
uint32_t crc32_8(const void* data, size_t len, uint32_t prev_value) {
    ...
    while (len >= 8) {
        ...
        uint32_t two = *cur++;
        crc = crc_lookup[7][(one) & 0xFF] ^ crc_lookup[6][(one >> 8) & 0xFF]
              ^ crc_lookup[5][(one >> 16) & 0xFF] ^ crc_lookup[4][(one >> 24) & 0xFF]
              ^ crc_lookup[3][(two) & 0xFF] ^ crc_lookup[2][(two >> 8) & 0xFF]
