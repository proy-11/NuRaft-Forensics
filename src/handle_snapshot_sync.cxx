...
ptr<req_msg> raft_server::create_sync_snapshot_req(peer& p, ulong last_log_idx, ulong term, ulong commit_idx) {
    ...
    if (!snp || sync_ctx->get_offset() == 0) {
        ...
        if (snp == nilptr || last_log_idx > snp->get_last_log_idx()) {
            ...
            if (snp) {
                p_er("last log idx %zu, snp last log idx %zu", last_log_idx, snp->get_last_log_idx());
            }
...
void raft_server::handle_install_snapshot_resp_new_member(resp_msg& resp) {
    ...
    if (snp_install_done) {
        ...
        sync_ctx->set_offset(resp.get_next_idx());
        p_db("continue to send snapshot to new server at offset %llu", resp.get_next_idx());
    }
    ...
    try {
        ...
        if (is_last_obj) {
            ...
            if (log_store_->compact(req.get_snapshot().get_last_log_idx())) {
                ...
                ptr<snapshot> new_snp = cs_new<snapshot>(req.get_snapshot().get_last_log_idx(),
                                                         req.get_snapshot().get_last_log_term(),
                                                         c_conf,
