...
private:
    ...
    static std::string
    dumpItem(LatencyItem* item, size_t max_filename_field = 0, uint64_t parent_total_time = 0, bool add_tab = true) {
        if (!max_filename_field) {
            max_filename_field = 32;
        }
        std::stringstream ss;
        ss << std::left << std::setw(max_filename_field) << getActualFunction(item->getStatName(), add_tab) << ": ";
        ss << std::right;
    ...
    static void dumpRecursive(std::stringstream& ss, DumpItem* dump_item, size_t max_name_len) {
        if (dump_item->itself) {
            if (dump_item->parent) {
                ss << dumpItem(dump_item->itself, max_name_len, dump_item->parent->getTotalTime());
            } else {
