...
int add_node_error_cases_test() {
    ...
    {   // Attempt to add server while previous one is in progress.
        ...
        size_t expected_cluster_size = 1;
...
int remove_and_then_add_test() {
    ...
    CHK_EQ(1, s3.raftServer->get_config()->get_servers().size());
...
int leader_election_with_aggressive_node_test() {
    ...
    CHK_EQ(num_attempts, MAX_ATTEMPTS);
...
int leadership_takeover_by_request_test() {
    ...
    CHK_FALSE( s3.raftServer->request_leadership() );
    // Send request.
    s3.fNet->execReqResp();
...
int follower_reconnect_test() {return 0;}
...
//     return 0;
// }
int snapshot_basic_test() {return 0;}
...
int custom_term_counter_test() {
    ...
    CHK_EQ( 0, s3.raftServer->get_term() );
