...
ptr<snapshot> generate_random_snapshot() {
    ptr<snapshot> snp(cs_new<snapshot>(long_val(rnd()), long_val(rnd()), generate_random_config(), long_val(rnd())));
    return snp;
}
...
int snapshot_test() {
    ...
    CHK_EQ(snp->get_last_config()->get_servers().size(), snp1->get_last_config()->get_servers().size());
    CHK_EQ(snp->get_last_config()->get_log_idx(), snp1->get_last_config()->get_log_idx());
    CHK_EQ(snp->get_last_config()->get_prev_log_idx(), snp1->get_last_config()->get_prev_log_idx());
    for (auto it = snp->get_last_config()->get_servers().begin(), it1 = snp1->get_last_config()->get_servers().begin();
...
int snapshot_sync_req_test(bool done) {
    ...
    CHK_EQ(snp->get_last_config()->get_servers().size(), snp2.get_last_config()->get_servers().size());
    CHK_EQ(snp->get_last_config()->get_log_idx(), snp2.get_last_config()->get_log_idx());
    CHK_EQ(snp->get_last_config()->get_prev_log_idx(), snp2.get_last_config()->get_prev_log_idx());
...
int snapshot_sync_req_zero_buffer_test(bool done) {
    ptr<snapshot> snp = generate_random_snapshot();
    ptr<snapshot_sync_req> sync_req(cs_new<snapshot_sync_req>(snp, long_val(rnd()), buffer::alloc(0), done));
    ptr<buffer> sync_req_buf(sync_req->serialize());
    ...
    CHK_EQ(snp->get_last_config()->get_servers().size(), snp3.get_last_config()->get_servers().size());
    CHK_EQ(snp->get_last_config()->get_log_idx(), snp3.get_last_config()->get_log_idx());
    CHK_EQ(snp->get_last_config()->get_prev_log_idx(), snp3.get_last_config()->get_prev_log_idx());
...
int custom_notification_msg_test(bool empty_context) {
    ...
    ptr<custom_notification_msg> dec_msg = custom_notification_msg::deserialize(*enc_msg);
...
int main(int argc, char** argv) {
    ...
    ts.doTest("snapshot_sync_req test", snapshot_sync_req_test, TestRange<bool>({true, false}));
    ts.doTest("snapshot_sync_req zero buffer test", snapshot_sync_req_zero_buffer_test, TestRange<bool>({true, false}));
    ts.doTest("log_entry test", log_entry_test);
    ts.doTest("custom_notification_msg test", custom_notification_msg_test, TestRange<bool>({true, false}));
