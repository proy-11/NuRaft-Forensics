...
void append_log(const std::string& cmd, const std::vector<std::string>& tokens) {
    ...
    ptr<buffer> new_log = d_raft_state_machine::enc_log({operand});
...
int main(int argc, char** argv) {
    if (argc < 3) d_raft_server::usage(argc, argv);

    d_raft_server::set_server_info(argc, argv);

    std::cout << "    -- Replicated Calculator with Raft --" << std::endl;
    std::cout << "                         Version 0.1.0" << std::endl;
    std::cout << "    Server ID:    " << stuff.server_id_ << std::endl;
    std::cout << "    Endpoint:     " << stuff.endpoint_ << std::endl;
    d_raft_server::init_raft(cs_new<d_raft_server::d_raft_state_machine>());
    d_raft_server::loop();
