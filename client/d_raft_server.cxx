                ...
                const std::vector<std::string>& tokens)
{
    ...
    d_raft_state_machine::op_type op = d_raft_state_machine::ADD;
    switch (cmd_char) {
    case '+':   op = d_raft_state_machine::ADD;   break;
    case '-':   op = d_raft_state_machine::SUB;   break;
    case '*':   op = d_raft_state_machine::MUL;   break;
    case '/':
        op = d_raft_state_machine::DIV;
        if (!operand) {
            std::cout << "cannot divide by zero" << std::endl;
            return;
        }
        break;
    default:    op = d_raft_state_machine::SET;   break;
    };

    // Serialize and generate Raft log to append.
    ptr<buffer> new_log = d_raft_state_machine::enc_log( {op, operand} );
