...
void create_server(int id, std::string ip, int server_port, int client_port, std::string byz) {
    ...
    <<" --port " << server_port << " --cport " << client_port <<" --byz " << byz;
...
void create_client(int port, std::string path) {
    std::ostringstream cbuf;
    cbuf << "client/d_raft_client --port " << port << " --path " << path;
...
int main(int argc, const char **argv)
{
    ...
    for(int i = 0; i < number_of_clients; i++) {
        std::cout << data["client"][i]["cport"] << " " << data["client"][i]["path"];
        clients.emplace_back(create_client, data["client"][i]["cport"], data["client"][i]["path"]); 
    }
